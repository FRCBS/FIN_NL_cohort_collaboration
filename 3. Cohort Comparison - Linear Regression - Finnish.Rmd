...
Based on "Ferritin Model" by Mikko Arvas (https://github.com/FRCBS/iron_measurement_comparisons/blob/master/src/ferritin_model_bayes.Rmd), this .Rmd contains the code to run Bayesian linear regression on ferritin levels for the Finnish populations. 
...
FolderName <- paste("../results/genpop_combined/ferritin_model_bayes", currentDate, sep="")
...
regression_data_men <- all_cohorts %>% 
  filter(Group == "Men") %>% 
  dplyr::select(ID, log_ferritin, Age, blood_vol, donation_count, log_last_donation, Cohort, iron_deficiency, Smoking, Group) %>%
...
regression_data_pre <- all_cohorts %>% 
  ...
  dplyr::select(ID, log_ferritin, Age, blood_vol, donation_count, log_last_donation, Cohort, iron_deficiency, Smoking, Group, Menstruation, HormonalContraception, PreviousChildbirth) %>%
  mutate(Age = scale(Age, scale = FALSE)[,1],
         donation_count = scale(donation_count, scale = FALSE)[,1],
         log_last_donation = scale(log_last_donation, scale = FALSE)[,1],
         blood_vol = scale(blood_vol, scale = FALSE)[,1],
...
# Model 4 [Corrected + Donation History + Smoking/Menstruation/OCC use/Childbirth ]
...
FinalForest_ModelsCombined$parameter <- gsub("CohortTHL", "General Population", FinalForest_ModelsCombined$parameter)
...
FinalForest_ModelsCombined_Pre$parameter <- gsub("CohortTHL", "General Population", FinalForest_ModelsCombined_Pre$parameter)
...
FinalForest_ModelsCombined_Pre$parameter <- gsub("HormonalContraceptionyes", "Hormonal contraception", FinalForest_ModelsCombined_Pre$parameter)
FinalForest_ModelsCombined_Pre$parameter <- as.factor(FinalForest_ModelsCombined_Pre$parameter)



desired_order <- c("General Population", "Age", "Blood volume (Nadler)", "Days since last blood donation (log transformed)", "Number of blood donations", "Number of blood donations (quadratic)", "Smoking", "Childbirth in the past", "Hormonal contraception", "Menstruation")
...
FinalForest_ModelsCombined_Post$parameter <- gsub("CohortTHL", "General Population", FinalForest_ModelsCombined_Post$parameter)
...
desired_order <- c("General Population", "Age", "Weight", "Blood volume (Nadler)", "Days since last blood donation (log transformed)", "Number of blood donations", "Number of blood donations (quadratic)", "Smoking", "Childbirth in the past", "Hormonal contraception", "Menstruation")
...
FinalForest_ModelsCombined_Pre_2$Group <- "Premenopausal Women"
FinalForest_ModelsCombined_Post_2 <- FinalForest_ModelsCombined_Post
FinalForest_ModelsCombined_Post_2$Group <- "Postmenopausal Women"
...
FinalForest_ModelsCombined_All_2$Group <- factor(FinalForest_ModelsCombined_All_2$Group, levels = c("Men", "Premenopausal Women", "Postmenopausal Women"))



desired_order_parameter <- c("General Population", "Age", "Blood volume (Nadler)", "Days since last blood donation (log transformed)", "Number of blood donations", "Number of blood donations (quadratic)", "Smoking", "Childbirth in the past", "Hormonal contraception", "Menstruation")
...
FinalForest_FIN <- FinalForest_ModelsCombined_All_2 %>% 
  add_column(Country = "FIN")

save(FinalForest_FIN, file = "~/proj/iron_measurement_comparisons/results/genpop_combined/ferritin_model_bayes/FinalForest_FIN.rdata")

saveRDS(FinalForest_FIN, file.path("~/proj/iron_measurement_comparisons/results/genpop_combined/ferritin_model_bayes/", "3. Cohort Comparison - Summary Data - Linear Regression.rds"))
